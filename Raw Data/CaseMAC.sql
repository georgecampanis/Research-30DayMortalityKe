...
Update MACv2
set MAC=[k_TotalMAC]
where [MACv2].[k_TotalMAC] >= 0.2 and  [MACv2].[k_TotalMAC] <=  2.5
...
;
WITH CTE AS (
  SELECT  rownum = ROW_NUMBER() over(PARTITION by  CaseID  order by  [DT] asc ), CaseID, DT, MAC FROM MACv2 where [MACv2].MAC is not null
)
...
(
(select [caseId], dt, MAC, [nxtTimeDiffSecs] from MACv2 where MAC is not Null and [nxtTimeDiffSecs] is not null ) m
Inner Join
(SELECT [caseId], Sum([nxtTimeDiffSecs]) as sumTime  FROM MACv2 group by caseId) t1
